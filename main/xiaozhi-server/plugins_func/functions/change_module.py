...
prompts = {"普通聊天模式":0, "心里辅导模式":1, "翻译模式":2}
change_module_function_desc = {
                "type": "function",
                "function": {
                    "name": "change_module",
                    "description": "当用户想切换模式/切换功能时调用,可选的模式有：[普通聊天模式,心里辅导模式,翻译模式]",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "module": {
...
def change_module(conn, module: str):
    ...
    new_module_id = prompts[module]
    conn.change_system_module(new_module_id)
    logger.bind(tag=TAG).info(f"切换模式:{module},模式名称:{module}")
    res = f"切换模式成功 进入{module}"
